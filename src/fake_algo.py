...
class FakeAlog:
    ...
    def __post_init__(self):
        self._hparam_dep = np.random.normal(size=(len(self.n_hparams) * 3)) + 0.1
    ...
    def step(self, hparams):
        ...
        perf_improvement = (np.dot(self._hparam_dep, featurized) +
                            np.random.normal(self._improvement_noise))
